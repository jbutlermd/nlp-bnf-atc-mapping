...
def fuzzy_merge(df_1, df_2, key1, key2, scorer=sm.Cosine, tokenizer=None, threshold=90, limit=2, distance=True):
    """
...
scorer_list = [sm.Cosine, sm.Jaccard, sm.Dice, sm.OverlapCoefficient, sm.TverskyIndex, sm.Cosine, sm.Jaccard, sm.Dice]
...
ngram = [2, 2, 2, 2, 2, 3, 3, 3]
column_width = {'A:A': 37, 'B:B': 40, 'C:C': 40, 'D:D': 19, 'E:E': 34, 'L:Q': 5}

with pd.ExcelWriter(os.path.join(config['DEFAULT']['output_dir'], 'pp-misspelling-method-token-based-results.xlsx')) as writer:
    for i, scorer in enumerate(scorer_list):
        ...
        merged_df = fuzzy_merge(bnf_df, atc_df, 'bnf_chemical_substance', 'ATC level name', threshold=90, limit=5,
                                scorer=scorer, tokenizer=sm.QgramTokenizer(qval=ngram[i], prefix_pad='^', suffix_pad='!'), distance=scorer_type[i])
