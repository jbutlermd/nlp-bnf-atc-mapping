...
def fuzzy_merge(df_1, df_2, key1, key2, scorer=sm.Cosine, tokenizer=None, hybrid=None, threshold=90, limit=2,
                distance=True):
    """
    ...
    m = df_1[key1].apply(
        lambda x: extract(str(x), s, scorer=scorer, tokenizer=tokenizer, hybrid=hybrid, limit=limit, distance=distance))
...
def extract(query, choices, processor=utils.full_process, scorer=sm.JaroWinkler,
            tokenizer=sm.WhitespaceTokenizer, hybrid=sm.MongeElkan, limit=2, distance=True, threshold=None):
...
scorer_list = [sm.Levenshtein, sm.Jaro, sm.JaroWinkler, sm.Editex, sm.Levenshtein, sm.Jaro, sm.JaroWinkler, sm.Editex]
hybrid_list = [sm.MongeElkan, sm.MongeElkan, sm.MongeElkan, sm.MongeElkan, sm.SoftTfIdf, sm.SoftTfIdf, sm.SoftTfIdf,
               sm.SoftTfIdf]
scorer_title = ["Levenshtein Monge-Elkan", "Jaro Monge-Elkan", "JaroWinkler Monge-Elkan", "Editex Monge-Elkan",
                "Levenshtein SoftTfIdf", "Jaro SoftTfIdf", "JaroWinkler SoftTfIdf", "Editex SoftTfIdf"]
scorer_type = [False, False, False, False, False, False, False, False]
ngram = [2, 2, 2, 2, 2, 2, 2, 2]
column_width = {'A:A': 37, 'B:B': 40, 'C:C': 40, 'D:D': 19, 'E:E': 34, 'L:Q': 5}

with pd.ExcelWriter(os.path.join(config['DEFAULT']['output_dir'], 'pp-misspelling-method-hybrid-results.xlsx')) as writer:
    for i, scorer in enumerate(scorer_list):
        ...
        #                         scorer=scorer,
        #                         tokenizer=sm.QgramTokenizer(qval=ngram[i], prefix_pad='^', suffix_pad='!'),
        #                         hybrid=hybrid_list[i], distance=scorer_type[i])

        merged_df = fuzzy_merge(bnf_df, atc_df, 'bnf_chemical_substance', 'ATC level name', threshold=90, limit=5,
                                scorer=scorer, tokenizer=sm.WhitespaceTokenizer(), hybrid=hybrid_list[i],
                                distance=scorer_type[i])
...
human_uptime = str(datetime.timedelta(seconds=int(uptime)))
print('Token-based similarity matching & merging is completed.')
