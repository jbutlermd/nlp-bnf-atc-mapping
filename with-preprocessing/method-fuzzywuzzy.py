...
def fuzzy_merge(df_1, df_2, key1, key2, scorer=fuzz.partial_ratio, threshold=90, limit=2):
    ...
    m = df_1[key1].apply(lambda x: process.extract(remove_stop_words(utils.full_process(str(x))), s, scorer=scorer, limit=limit))
...
with pd.ExcelWriter(os.path.join(config['DEFAULT']['output_dir'], 'misspelling-method-fuzzywuzzy-results.xlsx')) as writer:
